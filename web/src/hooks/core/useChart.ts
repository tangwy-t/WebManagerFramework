...
export function useChart(options: UseChartOptions = {}) {
  ...
  // 图表初始化核心逻辑
  const performChartInit = (options: EChartsOption, setOptionOpts?: SetOptionOpts) => {
  ...
  const initChart = (options: EChartsOption = {}, isEmpty: boolean = false, setOptionOpts?: SetOptionOpts) => {
    ...
    try {
      ...
      if (isContainerVisible(chartRef.value)) {
        // 容器可见，正常初始化
        if (initDelay > 0) {
          setTimeout(() => performChartInit(mergedOptions, setOptionOpts), initDelay)
        } else {
          performChartInit(mergedOptions, setOptionOpts)
        }
      } else {
        // 容器不可见，保存选项并设置监听器
  ...
  // 更新图表
  const updateChart = (options: EChartsOption, setOptionOpts?: SetOptionOpts) => {
    if (isDestroyed) return

    try {
      if (!chart) {
        // 如果图表不存在，先初始化
        initChart(options, false, setOptionOpts)
        return
      }
      chart.setOption(options, setOptionOpts)
    } catch (error) {
...
interface UseChartComponentOptions<T extends BaseChartProps> {
  ...
  /** setOption 第二参数的透传(如 { replaceMerge: ['series'] }),可选。
   * 也可传解析函数:每次更新时求值,按需决定本次是否整体替换系列——
   * 仅在系列列表变化时才 replaceMerge,日常数据刷新保持合并模式平滑过渡。 */
  setOptionOpts?: SetOptionOpts | (() => SetOptionOpts | undefined)
}
  ...
  const updateChart = () => {
    nextTick(() => {
      if (isEmpty.value) {
        ...
        // 有数据时移除空状态div并初始化图表
        emptyStateManager.remove()
        const opts = typeof setOptionOpts === 'function' ? setOptionOpts() : setOptionOpts
        initChart(generateOptions(), false, opts)
      }
    })
  }
