<template>
  <ElSubMenu v-if="hasChildren" :index="subMenuIndex" class="!p-0">
    <template #title>
...
<script lang="ts" setup>
  ...
  /**
   * 生成 el-menu 唯一 index。
   * 目录节点 path 可能为空，多目录撞出相同 index 会导致展开状态互相串扰，
   * 因此兜底用 MenuProcessor 生成的 name（dir-<id>），唯一稳定。
   */
  const subMenuIndex = computed(() => {
    if (props.item.path) return props.item.path
    if (typeof props.item.name === 'string' && props.item.name) return props.item.name
