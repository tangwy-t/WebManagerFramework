<template>
  <template v-for="(item, index) in filteredMenuItems" :key="getUniqueKey(item, index)">
    <ElSubMenu v-if="hasChildren(item)" :index="subMenuIndex(item)" :level="level">
      <template #title>
...
<script setup lang="ts">
  ...
  /**
   * 生成 el-menu 唯一 index。
   * 目录节点 path 可能为空（后端目录不带路由路径），若仅用 path/title 兜底，
   * 不同目录可能撞出相同 index，导致 el-menu 展开状态互相串扰（点一个展开另一个）。
   * name 由 MenuProcessor 生成（dir-<id>），唯一直观；path 存在时保持原语义不变。
   */
  const subMenuIndex = (item: AppRouteRecord): string => {
