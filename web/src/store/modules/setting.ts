...
export const useSettingStore = defineStore(
  ...
  () => {
    ...
    /** 自动锁屏空闲时长（分钟，0 = 不自动锁定，仅手动锁定） */
    const lockIdleMinutes = ref(SETTING_DEFAULT_CONFIG.lockIdleMinutes)
    ...
    /**
     * 设置自动锁屏空闲时长
     * @param minutes 空闲分钟数（0 = 不自动锁定）
     */
    const setLockIdleMinutes = (minutes: number) => {
      lockIdleMinutes.value = minutes
    }
