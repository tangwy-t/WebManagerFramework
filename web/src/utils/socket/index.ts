interface WebSocketOptions {
  ...
  onOpen?: (event: Event) => void // 每次连接成功（含重连）触发
  onClose?: (event: CloseEvent) => void // 当前连接关闭时触发（已销毁实例的延迟 close 不触发）
  pingMessage?: string | (() => string) // 应用层心跳消息，函数则每次调用取最新（默认 'ping'）
...
export default class WebSocketClient {
  ...
  private pingMessage: string | (() => string)
  ...
  static getInstance(options: WebSocketOptions): WebSocketClient {
    if (!WebSocketClient.instance) {
      ...
      // 首次创建即发起连接（与 url 变更时的行为保持一致）
      WebSocketClient.instance.init()
    } else {
      // 更新消息处理器
      WebSocketClient.instance.messageHandler = options.messageHandler
      // 同步更新 onOpen / onClose 钩子（重连/复用实例时以最新为准）
      WebSocketClient.instance.onOpen = options.onOpen
      WebSocketClient.instance.onClose = options.onClose
      // 如果提供了新的URL，则更新并重新连接
  ...
  /**
   * 幂等建连：已连接/正在连接时跳过；否则发起连接。
   * 覆盖重连次数耗尽（close(true)）后重新拉起连接的场景。
   */
  ensureConnection(): void {
    if (this.isConnecting || this.ws?.readyState === WebSocket.OPEN) return
    if (this.ws) {
      // CLOSING/CLOSED 的残留 socket，先清干净再建新连接
  ...
  private handleClose(event: CloseEvent): void {
    ...
    // 只有当前 socket 的关闭事件才通知调用方：destroyInstance 后旧实例的延迟 close
    // 不会误触发（新旧实例切换时 event.target 与 this.ws 不一致）
    const isCurrentSocket = event.target === this.ws
    this.isConnected = false
    this.isConnecting = false
  ...
  private startPing(): void {
    ...
    this.pingTimer = setInterval(() => {
      ...
      try {
        const payload =
          typeof this.pingMessage === 'function' ? this.pingMessage() : this.pingMessage
